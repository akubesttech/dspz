 <?php 
 $ID=$_GET['ID'];
 $user_reply = mysql_fetch_array(mysql_query("SELECT * from b_pms where pmID='$ID' and receiver='$student_RegNo'"))or die(mysql_error());
 $receipiant = $user_reply['sender']; $receimsg = $user_reply['message'];$recestatus = $user_reply['r_status'];
if($_SESSION['insidmsg']==$_POST['insidmsg'])
{
...
$query_adminrecord = mysql_query("SELECT * FROM admin WHERE username='$username' OR email='$username'  AND validate='1'")or die(mysql_error());$row_adminrecord = mysql_fetch_array($query_adminrecord); $num_checkadmin = mysql_num_rows($query_adminrecord); $email20 = $row_adminrecord['email']; $adminid = $row_adminrecord['admin_id']; $adminuser = $row_adminrecord['username'];

$query_student = mysql_query("SELECT * FROM student_tb WHERE RegNo ='$username' OR e_address='$username' AND verify_Data ='TRUE'")or die(mysql_error()); $num_row_student = mysql_num_rows($query_student);
		...
		$query_staff = mysql_query("SELECT * FROM staff_details WHERE usern_id='$username' OR email='$username' AND  r_status='2'")or die(mysql_error());$num_row_staff = mysql_num_rows($query_staff);
...
$sendmessage=mysql_query("INSERT into b_pms (sender, receiver,subject,message,therealtime,vartime,s_status,r_status) values('$student_RegNo','$receipiant','$subject','$msg','$date','$vartime','$recestatus','$sender_counts')") or die("Could not send message");
...
<div class="col-md-6 col-sm-6 col-xs-12 form-group has-feedback">
                     ...
                     <input type="text" class="form-control " name='username' id="username" placeholder="Username / Reg No / Email" value="<?php if($recestatus == '0'){
					echo getemail($user_reply['sender']);
					}else{
					 echo $user_reply['sender'];}?>"  readonly> </div>
