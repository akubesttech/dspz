...
if ($num_checkadmin < 1){ 
	message("ERROR:  The Recipiant UserID $username is Incorrect please Comfirm and try Again", "error");
		        redirect('message_m.php?view=c_Msg');
//$res="<font color='Red'><strong>ERROR:  The Recipiant UserID $username is Incorrect please Comfirm and try Again.</strong></font><br>";
			//	$resi=1;
}else{
$sendmessage=mysqli_query($condb,"INSERT into b_pms (sender, receiver,subject,message,therealtime,vartime,s_status,r_status) values('$student_RegNo','$adminid','$subject','$message','$date','$vartime','0','$sender_counts')") or die("Could not send message");
...
$sendmessage=mysqli_query($condb,"INSERT into b_pms (sender, receiver,subject,message,therealtime,vartime,s_status,r_status) values('$student_RegNo','$regnopass','$subject','$message','$date','$vartime','1','$sender_counts')") or die("Could not send message");
message("Your Message was Successfully Sent.", "success");
		        redirect('message_m.php'); 
...
$sendmessage=mysqli_query($condb,"INSERT into b_pms (sender, receiver,subject,message,therealtime,vartime,s_status,r_status) values('$student_RegNo','$usern_id','$subject','$message','$date','$vartime','2','$sender_counts')") or die("Could not send message");
	//$res="<font color='green'><strong>Your Message was Successfully Sent.</strong></font><br>";
	}}
message("Your Message was Successfully Sent..", "success");
		        redirect('message_m.php');
