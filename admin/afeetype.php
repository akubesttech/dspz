...
message("The Fee type <strong> $f_typea </strong>    Already Exist please Try Again.", "error");
			...
			mysqli_query($condb,"update  ftype_db set f_type='".safee($condb,$f_typea)."',d_desc='".safee($condb,$f_desca)."',f_category='".safee($condb,$fcate)."',status='".safee($condb,$status)."' where id ='".safee($condb,$_GET['id'])."'") or die(mysqli_error($condb));
...
<div class="x_panel">
                <div class="x_title">
                    ...
                    <h2> <?php echo ($_GET["id"] > 0) ? "Edit Fee Type " : "Add Fee Type "; ?></h2>
                ...
                <div class="x_content">
<?php $query_ft = mysqli_query($condb,"select * from ftype_db where id = '".safee($condb,$_GET['id'])."' ")or die(mysqli_error($condb));
 ...
 <div class="col-md-6 col-sm-6 col-xs-12 form-group has-feedback">
  ...
  <div class="col-md-6 col-sm-6 col-xs-12 form-group has-feedback" >
					    <label for="heard">Fee Category</label>
						  	  <select  name="fcate" id="fcate" class="form-control" ><?php if ($_GET["id"]> 0) { ?>
<option value="<?php echo $row_ft['f_category']; ?>"><?php echo getfcate($row_ft['f_category']); ?></option><?php }else{ ?> <option value="">Select Category</option><?php } ?>
			<option value="1">Fee</option><option value="2">Dues</option>
...
<?php if ($_GET["id"]> 0) { ?><option value="<?php echo $row_ft['status']; ?>"><?php if($row_ft['status'] =="1"){echo "compulsory";}else{ echo "Optional";} ?></option><?php }else{ ?> <option value="">Select Status</option>
                       <?php } ?>
