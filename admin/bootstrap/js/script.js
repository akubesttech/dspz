jQuery(document).ready( function($){
	...
	var desktopView = Backbone.View.extend({
		...
		loginProcess: function(evt){
			...
			$(_this).find(':input').attr('disabled',true);
			$(_this).find(':button').attr('disabled',false);
			$(_this).find(':button').html('<span class="fa fa-exchange"></span> &nbsp; Loading..');
				...
				$.ajax({
					...
					success: function( result ){
						if(result == 0){
							...
							$("#login-bottom").removeClass('hide').addClass('alert-success').html('<p><img src="img/logo/btn-ajax-loader.gif" /> &nbsp; Logging in......</p>');
		...
		checkLogin: function(url){
			$.ajax({
				...
				success: function( result ){
					if(result == 0){
						$("body").attr('class','login');
						$("#mainContext").load('Signin.php', function(){
							Backbone.history.navigate( '!/login' , true );
						});
					} else {
						$("body").removeClass('login');
