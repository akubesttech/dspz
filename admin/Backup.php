...
$query= mysqli_query($condb,"select * from schoolsetuptd ")or die(mysqli_error($condb));
							  $recordme = mysqli_fetch_array($query);
							  $databasename= ucwords($recordme['SchoolName']);

$status = FALSE;
if ( authorize($_SESSION["access3"]["bData"]["bdat"]["create"]) || 
authorize($_SESSION["access3"]["bData"]["bdat"]["edit"]) || 
authorize($_SESSION["access3"]["bData"]["bdat"]["view"]) || 
authorize($_SESSION["access3"]["bData"]["bdat"]["delete"]) ) {
 $status = TRUE;
}
if ($status === FALSE) {
...
if(mysqli_connect_errno($condb)) {
  ...
  $tables = array();
    ...
    $date = date('m-d-Y'); 
$backup_file_name = "Database Of $databasename ".$date.".sql"; 
    backup_database($condb, $tables, $backup_file_name);
  }}
?>

<?php
function backup_database($condb, $tables = "", $backup_file_name) {

  if(empty($tables)) {
    $tables_in_database = mysqli_query($condb, "SHOW TABLES");
    ...
    foreach($tables as $table) {
      if(mysqli_num_rows(mysqli_query($condb, "SHOW TABLES LIKE '".$table."'")) == 1) {
  ...
  foreach($tables as $table) {
    $result        = mysqli_query($condb, "SELECT * FROM ".$table);
    $no_of_columns = mysqli_num_fields($result);
    $no_of_rows    = mysqli_num_rows($result);

    //Get the query for table creation
    $table_query     = mysqli_query($condb, "SHOW CREATE TABLE ".$table);
