...
<?php
$query_userrecord = mysqli_query($condb,"select * from staff_details where usern_id = '$admin_username'")or die(mysqli_error($condb));
$checkuser = mysqli_num_rows($query_userrecord);
...
$moe = $_POST['moe'] ; $bname = $_POST['bname'] ;$acctname = $_POST['acctname'] ; $scode = $_POST['scode'] ; $getaimage = $_POST['image'];
//$image_find = $_POST['pic'] ; $Cverify = $_POST['Cverify'] ; $time=date('l jS \of F Y h:i:s A');
//$webaddress=$_SERVER['HTTP_HOST'];

//$query = mysql_query("select * from staff_details where usern_id = '$sid' ")or die(mysqli_error($condb));
...
//$res="<font color='red'><strong>This Staff Record Already Exist,Try Again!</strong></font><br>";
		...
		if(!preg_match("/([\w\-]+\@[\w\-]+\.[\w\-]+)/", $eaddress)){
			message("ERROR: Please! Provide a valid Email Address", "error");
		        redirect('user_Private.php?view=UPRO');
...
if($checkuser > 0){
mysqli_query($condb,"update staff_details  set sname='$sname',mname='$mname',oname='$oname',mstatus='$Mstatus',hobbies='$hobbies',phone='$phone',email='$eaddress',paddress='$paddress',caddress='$caddress',town='$stown',lga='$lga',state='$state',nation='$nation',job_desc='$jdesc',cos='$Cstudy',s_dept='$dept1',b_name='$bname',b_acct_name='$acctname',b_acct_num='$acctnum',b_sort='$scode' where usern_id='$admin_username'")or die(mysqli_error($condb));
...
if ($email_check > 1){ 
		message("ERROR:  This Email Address is already in use inside our system. Please try another", "error");
		        redirect('user_Private.php?view=UPRO');}else{
mysqli_query($condb,"insert into staff_details (sname,mname,oname,mstatus,hobbies,phone,email,paddress,caddress,town,lga,state,nation,heq,cos,b_name,b_acct_name,b_acct_num,b_sort,usern_id,password,r_status,image) values('".safee($condb,$sname)."','".safee($condb,$mname)."','".safee($condb,$oname)."','".safee($condb,$Mstatus)."','".safee($condb,$hobbies)."','".safee($condb,$phone)."','".safee($condb,$eaddress)."','".safee($condb,$paddress)."','".safee($condb,$caddress)."','".safee($condb,$stown)."','".safee($condb,$lga)."','".safee($condb,$state)."','".safee($condb,$nation)."','".safee($condb,$heq)."','".safee($condb,$Cstudy)."','".safee($condb,$bname)."','".safee($condb,$acctname)."','".safee($condb,$acctnum)."','".safee($condb,$scode)."','".safee($condb,$admin_username)."','".safee($condb,$admin_username)."','2','".safee($condb,$getaimage)."')")or die(mysqli_error($condb));
mysqli_query($condb,"update admin  set firstname ='$sname',lastname='$mname',phone='$phone',email='$eaddress' where username ='$admin_username'")or die(mysqli_error($condb));}
}
		


//mysql_query("insert into activity_log (date,username,action) values(NOW(),'$admin_username','Staff Details of $sname $oname with staff id $sid  was Updated')")or die(mysqli_error($condb)); 
// ob_start();
...
<input type="hidden" name="image" value="<?php echo $row_change['adminthumbnails'];?>" />
                      
                      <span class="section">User Profile
 </span>
 ...
 //if ($row['adminthumbnails']==NULL ){print "./uploads/NO-IMAGE-AVAILABLE.jpg";}else{print $row['adminthumbnails'];}
		if ($exists > 0 ){print $row['adminthumbnails']; }else{ print "uploads/NO-IMAGE-AVAILABLE.jpg";}			  
	...
	<?php
                      ...
                      <ul class="list-unstyled user_data">
                        ...
                        <li>
                          ...
                          <li>
                          <i class="fa fa-briefcase user-profile-icon"></i> <?php echo $row_userrecord['s_dept'];?>
                        </li>
                        <li class="m-top-xs">
...
<div class="col-md-4 col-sm-4 col-xs-12 form-group has-feedback">
					  <label for="heard">Surname </label>
                          ...
                          <input type="text" class="form-control "  name='sname' id="sname"  value="<?php echo $row_userrecord['sname']; ?>" required="required" > </div>
                          
                          
<div class="col-md-4 col-sm-4 col-xs-12 form-group has-feedback">
					  <label for="heard">Middle Name</label>
                      
                          <input type="text" class="form-control " name="mname" id="mname" maxlength="20"  value="<?php echo $row_userrecord['mname']; ?>"   required="required" > </div>

                          
                          
 <div class="col-md-4 col-sm-4 col-xs-12 form-group has-feedback">
						  	  <label for="heard">Other Name</label>
                                         <input type="text" class="form-control " name="oname" id="oname" maxlength="20"  value="<?php echo $row_userrecord['oname']; ?>"    >
                      </div>
                      ...
                      <div class="col-md-4 col-sm-4 col-xs-12 form-group has-feedback">
                       ...
                       <div class="col-md-4 col-sm-4 col-xs-12 form-group has-feedback">
						  	  <label for="heard">Course Studied</label>
                            	  <input type="text" class="form-control " name='cstudy' id="cstudy" value="<?php echo $row_userrecord['cos']; ?>" >
                      </div>
