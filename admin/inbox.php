                  ...
                  <div class="x_content">
                    ...
                    <table id="datatable-buttons" class="table table-striped table-bordered">
                      ...
                      <tbody>
                       <?php
                         if($_GET['unread'] == "236"){
$user_query = mysqli_query($condb,"select * from b_pms where receiver='$session_id' and hasread = '0' order by therealtime DESC")or die(mysqli_error($condb));
}else{
$user_query = mysqli_query($condb,"select * from b_pms where receiver='$session_id' order by therealtime DESC")or die(mysqli_error($condb));
}
		...
		$exists2 = imgExists("../Student/".$studentimagein['images']);
   $exists3 = imgExists("./".$staffimagevin['image']);
   $exists4 = imgExists($row['adminthumbnails']);
													?>
                        <tr>
                        	<td width="30">
...
<img src="<?php //if($studentimagein['images']==NULL ){ print "./uploads/NO-IMAGE-AVAILABLE.jpg"; }else{ print $studentimagein['images'];}
	if ($exists2 > 0 ){print "../Student/".$studentimagein['images'];}else{ echo "./uploads/NO-IMAGE-AVAILABLE.jpg";}
	
	 ?>" style="width:50px;height:30px;" class="avatar" alt="image" />
	<?php	}elseif($row_f['r_status']=='2'){ ?>
	<img src="<?php //if($staffimagevin['image']==NULL ){ print "./uploads/NO-IMAGE-AVAILABLE.jpg";}else{print $staffimagevin['image'];}
	if ($exists3 > 0 ){print "./".$staffimagevin['image'];}else{ echo "./uploads/NO-IMAGE-AVAILABLE.jpg";}
	 ?>" style="width:50px;height:30px;" class="avatar" alt="image" />	<?php }else{ ?>
<img src="<?php //if($row['adminthumbnails']==NULL ){print "./uploads/NO-IMAGE-AVAILABLE.jpg";}else{ print $row['adminthumbnails'];}
if ($exists4 > 0 ){print $row['adminthumbnails'];}else{ echo "uploads/NO-IMAGE-AVAILABLE.jpg";}
