...
$query= mysqli_query($condb,"select * from schoolsetuptd ")or die(mysqli_error($condb));
	...
	.footer {
		font-size: 11px;
          ...
          <strong> <?php echo $_GET['semester'];?> Semester Lecture Time Table For <?php echo getdeptc($_GET['dept1'])." Department , ";?><?php echo getlevel($_GET['level'],$class_ID);?> level .</strong>
                  </div>
...
$viewutme_query = mysqli_query($condb,"select time_id  , day , time ,duration,venue,course from lecttime_tb where t_dept = '".safee($condb,$depart)."' and session='".safee($condb,$_GET['session'])."' and t_level='".safee($condb,$_GET['level'])."' and semester ='".safee($condb,$_GET['semester'])."'  order by session DESC ")or die(mysqli_error($condb)); ?>
...
if($num_row < 1){
echo "<td colspan='10' style='text-align:centre;' ><strong>No Lecture Time Table Added for ".getdeptc($_GET['dept1'])." Department . </strong></td>";}
 ?>
 						
<div class="btn-group" id="divButtons" name="divButtons">
<button  name="goback"  id="goback" type='button' onClick="window.location.href='vtime_man.php?view=sa_time';" class="btn btn-primary" title="Click to Go Back" ><i class="fa fa-back"></i> Go Back</button>&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" value="Print" onclick="tablePrint();" class="btn btn-default">
                      	 </div>
                    </table>
