...
if ($row_fac>1){
message("The ".$SCategory." Entered  Already Exist Try Again!", "error");
		       redirect('add_Faculty.php?id='.$get_RegNo);

				//echo "<script>alert('Applicationform record inserted sucessfully..');</script>";
				}elseif(!ctype_digit($fac_phone)){
				message("Incorrect Format For Phone Number it should be a Digit!", "error");
		       redirect('add_Faculty.php?id='.$get_RegNo);
...
mysqli_query($condb,"insert into activity_log (date,username,action) values(NOW(),'$admin_username','Faculty Titled $fac_name was Add')")or die(mysqli_error($condb)); 
 ob_start();
 message("$fac_name has Updated successfully!", "success");
...
<div class="x_panel">
                ...
                <div class="x_content">
<?php
								$query_f = mysqli_query($condb,"select * from faculty where fac_id ='$get_RegNo' ")or die(mysqli_error($condb));
...
?> </span>
 ...
 <div class="col-md-6 col-sm-6 col-xs-12 form-group has-feedback">
						  	  <label for="heard"><?php echo $SCategory; ?> Name</label>
                            	  <input type="text" class="form-control " name='fac_name' id="fac_name" value="<?php echo $row_f['fac_name']; ?>"  required="required">
                      </div>
 ...
 <div class="col-md-6 col-sm-6 col-xs-12 form-group has-feedback">
						  	  <label for="heard"><?php echo $SCategory; ?> Office Phone </label>
                            	  <input type="text" class="form-control " name='fac_phone' id="fac_phone" value="<?php echo $row_f['fac_phone']; ?>" onkeypress="return isNumber(event);"  >
                      </div>
					  ...
					  <label for="heard">Dean of The <?php echo $SCategory; ?> </label>
                          ...
                          <select name='fac_dean' id="fac_dean" class="form-control" >
                            <option value="<?php echo $row_f['fac_dean']; ?>"><?php echo getdean($row_f['fac_dean']); ?></option>
...
?>
                      ...
                      <div  class="col-md-6 col-sm-6 col-xs-12 form-group has-feedback>
                        <div class="col-md-6 col-md-offset-3">
                       <?php  if (authorize($_SESSION["access3"]["sConfig"]["afac"]["edit"])){ ?>
                        <button  name="addfac"  id="addfac"  class="btn btn-primary col-md-4" title="Click Here to Edit <?php echo $SCategory; ?> Details" ><i class="fa fa-sign-in"></i> Edit <?php echo $SCategory; ?></button> <?php } ?>
