...
$Day = $_POST['day'];
//$Day= DateTime::createFromFormat('d/m/Y', $Day2)->format('d-m-Y');
...
//$checkcourse =  mysql_query("select assigned, dept, course  from course_allottb where course = '".mysql_real_escape_string($salot_cos)."'  AND assigned = '".mysql_real_escape_string($salot_id)."' AND dept = '".mysql_real_escape_string($salot_dept)."'");
//$row4 =mysql_fetch_array($checkcourse);
 //$numrow4=mysql_num_rows($checkcourse);

$query_ltime = mysqli_query($condb,"select * from lecttime_tb where t_level ='".safee($condb,$salot_los)."' AND course = '".safee($condb,$salot_cos)."' AND day = '".safee($condb,$Day)."' AND time = '".safee($condb,$tstart2)."' AND duration = '".safee($condb,$duration)."'")or die(mysqli_error($condb));
$query_ltime2 = mysqli_query($condb,"select * from lecttime_tb where t_level ='".safee($condb,$salot_los)."' AND course = '".safee($condb,$salot_cos)."' AND day = '".safee($condb,$Day)."' AND time = '".safee($condb,$tstart2)."' ")or die(mysqli_error($condb));
$row_ltime=mysqli_fetch_array($query_ltime); $count=mysqli_num_rows($query_ltime); $count2=mysqli_num_rows($query_ltime2);
if($count>0){	

ob_start();
	message("Please This Lecture Time Set Already Exist In our Database  Or The Lecture Time / Course is Clashing.", "error");
		       redirect('lecture_time.php?view=l_add');
		       }elseif($count2>0){	
	...
	message("Lecture Period Already Occupy By another Course try Another", "error");
		       ...
		       message("Lecture End Time should not be lessthan Start Time.", "error");
		       redirect('lecture_time.php?view=l_add');
}else{
$dbsettime = "INSERT INTO lecttime_tb (t_dept,t_level,semester,session,day,time,duration,course,venue,prog) values ('".safee($condb,$salot_dept)."', '".safee($condb,$salot_los)."','".safee($condb,$salot_sem)."', '".safee($condb,$salot_session)."', '".safee($condb,$Day)."', '".safee($condb,$tstart2)."','".safee($condb,$duration)."','".safee($condb,$salot_cos)."','".safee($condb,$venuet)."','".safee($condb,$class_ID)."')";
	$Querysettime = mysqli_query($condb,$dbsettime) or die ("Couldn't set Lecture time For $salot_cos.");
		message("Lecture Time For <b> $salot_cos </b> was successfully Added", "success");
		       redirect('lecture_time.php?view=load01');
}
}
?>
...
<input type="hidden" name="insidtime" value="<?php echo $_SESSION['insidtime'];?> " />
                      ...
                      <span class="section">Set New Lecture Time</span>
...
{if($_GET['loadfac'] ==$rsblocks['fac_id'] ){echo "<option value='$rsblocks[fac_id]' selected>$rsblocks[fac_name]</option>";
                      ...
                      <div class="col-md-3 col-sm-3 col-xs-12 form-group has-feedback">
                       <label for="heard">Courses</label><select name='cos' id="cosload" class="form-control"  >
...
<option value="Second">Second</option></select>
                       ...
                       <div class="col-md-3 col-sm-3 col-xs-12 form-group has-feedback">
						  	  <label for="heard">Day</label>
<!--<input  type="text" name="day" size="25"   class="w8em format-d-m-y highlight-days-67 range-middle-today" id="ed"   readonly="readonly" style="height:32px;" autocomplete="off" > --!>
...
//$arrtime2 = array("6.00 AM","7.00 AM","8.00 AM","9.00 AM","10.00 AM","11.00 AM","12.00 PM","01.00 PM","02.00 PM","03.00 PM","04.00 PM","05.00 PM","06.00 PM","07.00 PM","08.00 PM");foreach($arrtime2 as $val2){echo "<option value='$val2'>$val2</option>";}?></select></div>
                  ...
                  <div class="col-md-6 col-sm-6 col-xs-12 form-group has-feedback">
                      ...
                      <a href='javascript:void(0);' onclick="window.open('lecture_time.php?view=load01','_self')" class="btn btn-primary"  id="delete02" data-placement="right" title="Click to View Details" ><i class="fa fa-file icon-large"></i> View Details</a>
