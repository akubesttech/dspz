...
$user_reply = mysqli_fetch_array(mysqli_query($condb,"SELECT * from b_pms where pmID='$ID' and receiver='$session_id'"))or die(mysqli_error($condb));
 $receipiant = $user_reply['sender']; $receimsg = $user_reply['message'];$recestatus = $user_reply['status']; $user_reply['r_status'];
...
<div class="col-md-6 col-sm-6 col-xs-12 form-group has-feedback">
					  <label for="heard" id="c_title">Username of Recipient : </label>
					   <input type="hidden" class="form-control " name='username' id="username" value="<?php echo $user_reply['sender']; ?>" >
                     <input type="text" class="form-control " name='username1' id="username1" placeholder="Username / Reg No / Email" value="<?php 	if($user_reply['r_status']=='1'){echo strtoupper($user_reply['sender']);}elseif($user_reply['r_status']=='2'){echo strtoupper($user_reply['sender']);}else{echo strtoupper(getemail($user_reply['sender']));	}?>"  readonly> </div>
