 <?php 
 $ID=$_GET['ID'];
 $user_reply = mysqli_fetch_array(mysqli_query($condb,"SELECT * from b_pms where pmID='$ID' and receiver='$staff_id'"))or die(mysqli_error($condb));
...
$sendmessage=mysqli_query($condb,"INSERT into b_pms (sender, receiver,subject,message,therealtime,vartime,s_status,r_status) values('$staff_id','$receipiant','$subject','$msg','$date','$vartime','$recestatus','$sender_counts')") or die("Could not send message");
	        //echo "<script>alert('Your Message Reply was Successfully.');</script>";
	//	echo "<script>window.location.assign('message_m.php');</script>";
			message("Your Message Reply was Successfully.", "success");
		        redirect('message_m.php');
					}
